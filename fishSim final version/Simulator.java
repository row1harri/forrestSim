...
public class Simulator
{
    ...
    private static final double STINGRAY_CREATION_PROBABILITY = 0.03;
    ...
    private void changeSeason(){
        if ((step-60) % 60==0){
            field.changeIsColdSeason();
        }   
    }
    
    private void seasonalAct(){
        if(field.getIsColdSeason()){
            for(Animal animal: animals ){ 
                animal.changeDigestionSpeed(0);//digestion is slower
            }
        }
        else{
            for(Animal animal: animals ){ 
                animal.changeDigestionSpeed(1);//digestion is faster
            }
        }
    }
}